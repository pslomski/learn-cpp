...
{
public:
    explicit ListError(char const* const message) noexcept : std::runtime_error(message) {}
};
...
public:
    ...
    List(List&& lst) noexcept : head(std::exchange(lst.head, nullptr)), tail(std::exchange(lst.tail, nullptr)) {}
...
private:
    ...
    public:
        ...
        // Node* prev{ nullptr };
        Node* next{nullptr};
        static Node* create(const T& value) { return new Node(value); }
    };
    Node* head{nullptr};
    Node* tail{nullptr};
